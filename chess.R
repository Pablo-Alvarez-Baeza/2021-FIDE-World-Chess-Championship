...
# This data frame will be used to quickly see the details of each game.
df_text <-  df |> 
  mutate(date = ymd(date),
         ...
         result_text = case_when(result == "1/2-1/2" ~ "—", # Draw
...
moves <- str_extract_all(df$movetext, "[A-Za-z]\\S+") |> 
  ...
  as.data.frame() |> 
  group_by(round) |>  
  mutate(move_color = factor(if_else(row_number() %% 2 == 1, "white", "grey50"),
                             levels = c("white", "grey50"), ordered = TRUE),
...
plot <- moves |> 
  ...
  labs(x = NULL,
       y = NULL,
       title = "FIDE CHESS WORLD CHAMPIONSHIP DUBAI 2021",
       subtitle = "CARLSEN 7½ - 3½ NEPO",
...
plot +
  ...
  annotate("segment", x = 0.950, y = 0, xend = 1.150, yend = 0, color = "white") +
  annotate("segment", x = 1.250, y = 0, xend = 1.450, yend = 0, color = "white") +
  annotate("segment", x = 1.550, y = 0, xend = 1.750, yend = 0, color = "white") +
  annotate("segment", x = 1.850, y = 0, xend = 2.050, yend = 0, color = "white") +
  annotate("segment", x = 2.150, y = 0, xend = 2.350, yend = 0, color = "white") +
  annotate("segment", x = 2.450, y = 0, xend = 2.650, yend = 0, color = "white") +
  annotate("segment", x = 2.750, y = 0, xend = 2.950, yend = 0, color = "white") +
  annotate("segment", x = 3.050, y = 0, xend = 3.250, yend = 0, color = "white") +
  annotate("segment", x = 3.350, y = 0, xend = 3.550, yend = 0, color = "white") +
  annotate("segment", x = 3.650, y = 0, xend = 3.850, yend = 0, color = "white") +
  annotate("segment", x = 3.950, y = 0, xend = 4.150, yend = 0, color = "white") +
  ...
  annotate(geom = "richtext",
           ...
           <span>**Carlsen**</span><br><span style='color:grey50'>**|**<span>",
